...
async function onClearAllComments() {
  ...
  const { error } = await client.from('comments').delete().gt('id', 0);
...
async function onClearAllTickets() {
  ...
  const delMsgs = await client.from('support_messages').delete().gt('id', 0);
  ...
  const delT = await client.from('support_tickets').delete().gt('id', 0);
...
async function onClearAllIpNames() {
  ...
  const { error } = await client.from('ip_names').delete().gt('id', 0);
  if (error) { setSettingsStatus('❌ Ошибка: ' + error.message, true); return; }
...
async function onClearAllDangerous() {
  ...
  try {
    ...
    const { error: e1 } = await client.from('comments').delete().gt('id', 0);
    if (e1) throw new Error('Комментарии: ' + e1.message);
    
    setSettingsStatus('📋 Удаление сообщений тикетов...');
    const { error: e2 } = await client.from('support_messages').delete().gt('id', 0);
    if (e2) throw new Error('Сообщения тикетов: ' + e2.message);
    
    setSettingsStatus('🎫 Удаление тикетов...');
    const { error: e3 } = await client.from('support_tickets').delete().gt('id', 0);
    if (e3) throw new Error('Тикеты: ' + e3.message);
    
    setSettingsStatus('👤 Сброс закреплённых ников...');
    const { error: e4 } = await client.from('ip_names').delete().gt('id', 0);
