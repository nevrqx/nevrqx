...
async function loadHome() {
  ...
  // Загружаем все посты для подсчета и фильтрации
  const { data, error } = await client
    .from('posts')
    .select('*')
    .eq('status', 'published')
  ...
  state.totalPosts = filteredItems.length;
  
  // Сброс на первую страницу при новом поиске
  if (q !== state.currentSearchQuery) {
    state.currentPage = 1;
  }
  
  renderPostsWithPagination(filteredItems);
}

function renderPostsWithPagination(allItems) {
  const totalPages = Math.ceil(allItems.length / state.postsPerPage);
...
function goToPage(pageNum) {
  ...
  // Повторно фильтруем и отображаем
...
const handleGlobalSearch = () => {
  ...
  state.currentSearchQuery = q;
  state.totalPosts = items.length;
  state.currentPage = 1; // Сброс на первую страницу при поиске
  
  renderPostsWithPagination(items);
};
...
async function loadMyTickets() {
  ...
  state.totalTickets = state.myTickets.length;
  // Keep page within bounds
  const maxTicketPages = Math.max(1, Math.ceil(state.totalTickets / state.ticketsPerPage));
  if (state.currentTicketPage > maxTicketPages) state.currentTicketPage = maxTicketPages;
  ...
  renderMyTicketsWithPagination(state.myTickets);
  setTimeout(updateProfileTabCounts, 0);
}
...
function renderMyTicketsWithPagination(allItems) {
  const totalPages = Math.ceil((allItems?.length || 0) / state.ticketsPerPage) || 1;
  const start = (state.currentTicketPage - 1) * state.ticketsPerPage;
  const pageItems = (allItems || []).slice(start, start + state.ticketsPerPage);
  renderMyTickets(pageItems);
  renderMyTicketsPagination(totalPages);
}
